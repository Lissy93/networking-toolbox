<script lang="ts">
  ...
  async function copyResults() {
    ...
    let text = `${$t('diagnostics/email-spf-check.copy.header', { domain })}\n`;
    text += `${$t('diagnostics/email-spf-check.copy.generatedAt', { timestamp: new Date().toISOString() })}\n\n`;

    if (results.record) {
      text += `${$t('diagnostics/email-spf-check.copy.recordLabel')}\n${results.record}\n\n`;
    }

    if (results.emailAnalysis) {
      text += `${$t('diagnostics/email-spf-check.copy.emailAnalysisLabel')}\n`;
      text += `  ${$t('diagnostics/email-spf-check.copy.riskLevel')} ${results.emailAnalysis.deliverabilityRisk}\n`;
      text += `  ${$t('diagnostics/email-spf-check.copy.hardFailLabel')} ${results.emailAnalysis.hasHardFail ? $t('diagnostics/email-spf-check.copy.yes') : $t('diagnostics/email-spf-check.copy.no')}\n`;
      text += `  ${$t('diagnostics/email-spf-check.copy.softFailLabel')} ${results.emailAnalysis.hasSoftFail ? $t('diagnostics/email-spf-check.copy.yes') : $t('diagnostics/email-spf-check.copy.no')}\n`;
      text += `  ${$t('diagnostics/email-spf-check.copy.allowsAllLabel')} ${results.emailAnalysis.allowsAll ? $t('diagnostics/email-spf-check.copy.yes') : $t('diagnostics/email-spf-check.copy.no')}\n\n`;
    }
    ...
    if (expandedResults.expanded) {
      text += `${$t('diagnostics/email-spf-check.copy.expandedAnalysisLabel')}\n`;
      text += `  ${$t('diagnostics/email-spf-check.copy.totalLookupsLabel')} ${expandedResults.lookupCount || 0}\n`;
      text += `  ${$t('diagnostics/email-spf-check.copy.mechanismsLabel')} ${expandedResults.expanded.mechanisms.join(', ')}\n`;
      if (expandedResults.expanded.includes.length > 0) {
        text += `  ${$t('diagnostics/email-spf-check.copy.includesLabel')} ${expandedResults.expanded.includes.map((inc) => inc.domain).join(', ')}\n`;
      }
    }
