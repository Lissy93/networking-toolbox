<script lang="ts">
  ...
  const examples = [
    { domain: 'google.com', description: t('diagnostics.dns-dmarc-check.examples.items.google.description') },
    { domain: 'github.com', description: t('diagnostics.dns-dmarc-check.examples.items.github.description') },
    { domain: 'microsoft.com', description: t('diagnostics.dns-dmarc-check.examples.items.microsoft.description') },
    { domain: 'paypal.com', description: t('diagnostics.dns-dmarc-check.examples.items.paypal.description') },
    { domain: 'amazon.com', description: t('diagnostics.dns-dmarc-check.examples.items.amazon.description') },
    { domain: 'salesforce.com', description: t('diagnostics.dns-dmarc-check.examples.items.salesforce.description') },
  ];
  ...
  async function copyResults() {
    ...
    if (results.parsed) {
      ...
      text += `  ${t('diagnostics.dns-dmarc-check.copy.dkimAlignmentLabel')} ${p.alignment.dkim} (${p.alignment.dkim === 's' ? t('diagnostics.dns-dmarc-check.copy.alignmentStrict') : t('diagnostics.dns-dmarc-check.copy.alignmentRelaxed')})\n`;
      text += `  ${t('diagnostics.dns-dmarc-check.copy.spfAlignmentLabel')} ${p.alignment.spf} (${p.alignment.spf === 's' ? t('diagnostics.dns-dmarc-check.copy.alignmentStrict') : t('diagnostics.dns-dmarc-check.copy.alignmentRelaxed')})\n`;
      ...
      <div class="examples-grid">
        {#each examples as example, i (i)}
          <button
            ...
            use:tooltip={t('diagnostics.dns-dmarc-check.examples.items.google.tooltip', {
              domain: example.domain,
                ...
                <div class="policy-value {getAlignmentColor(parsed.alignment.dkim)}">
                  <span class="policy-text"
                    >{parsed.alignment.dkim === 's'
                      ...
                      : t('diagnostics.dns-dmarc-check.results.policyConfiguration.alignment.relaxed')}</span
                  >
                  <span class="policy-description">
                    {parsed.alignment.dkim === 's'
